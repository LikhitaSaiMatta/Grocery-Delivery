        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div>
                    ...
                    <span className="text-xl font-semibold">{footerData.brand.name}
                    </span>
                    </Link>
                    ...
                    <div className="flex gap-3">
                        {footerData.brand.socials.map((social, i)=>(
                            <a key={i} href={social.link} className="size-9 rounded-lg
                            bg-white/10 flex-center hover:bg-white/2"><social.icon
                            className="size-4"/>
                            </a>
                        ))}
                    </div>
                        ...
                        {section.links.map((link, i)=>(
                            <li key={i}>
                                {link.to ? (
                                    ...
                                    text-white/70 hover:text-white">{link.label}
                                    </a>
                                )}
                            </li>
                        ))}
                ...
                {/* Contact */}
                <div>
                    <h3 className="text-sm font-semibold uppercase mb-4">Contact
                    Us</h3>
            ...
            sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-4">
                    {footerData.bottom.links.map((link, i)=>(
                        <a key={i} href={link.href} className="text-xs text-white/
                        50 hover:text-white/70">{link.label}</a>
                    ))}
