...
const CartSidebar = () => {
  ...
  return (
    <>
        ...
        bg-white z-50 shadow-2xl flex flex-col animate-slide-in-right">
            ...
            border-app-border">
                <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 text-xs font-semibold bg-app-cream
                    rounded-full">{items.length} items</span>
                </div>
                    ...
                    items.map((item)=>(
                        <div key={item.product._id} className="flex gap-3 bg-app-cream/60 rounded-xl p-3">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {currency}{item.product.price.toFixed(2)} / {item.product.unit}</p>
                                <div className="flex items-center justify-between mt-2">
                                    <div className="flex items-center gap-1.5">
                                        <button onClick={()=> updateQuantity(item.product._id, item.quantity - 1)}
                                        className="size-7 rounded-lg bg-white
                                        border border-app-border flex-center">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-sm font-semibold">
                                            {currency}{(item.product.price * item.
                                            quantity).toFixed(2)}
                                        </span>
            ...
            {items.length > 0 && (
                <div className="p-5 border-t border-app-border space-y-3">
                    ...
                    hover:bg-app-orange-dark transition-colors
